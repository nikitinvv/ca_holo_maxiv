...
class Rec:
    ...
    def gradientF_q(self, q, dref):
        tmp = self.D(q[np.newaxis])        
        td = dref * (tmp / (np.abs(tmp) + self.eps))
        ...
        if i % self.vis_step == 0 and self.vis_step != -1 and self.show:
            ...
            for k in range(0, npp * 2):
                ...
                errt[k] = self.minF(tmp, d) + \
                    self.lam*np.linalg.norm(self.G(qt))**2 + \
                    np.linalg.norm(np.abs(self.D(qt[np.newaxis]))-dref)**2

            t = alpha * (np.arange(2 * npp)) / (npp - 1)
            tmp = self.fwd(ri,r,psi,q)
            errt2 = self.minF(tmp, d) + \
                self.lam*np.linalg.norm(self.G(q))**2 + \
                np.linalg.norm(np.abs(self.D(q[np.newaxis]))-dref)**2
            errt2 = errt2 - top * t + 0.5 * bottom * t**2

            plt.plot(
                alpha * np.arange(2 * npp) / (npp - 1),
                errt,
                ".",
                label="approximation",
            )
    ...
    def vis_debug(self, vars, i):
        ...
        if i % self.vis_step == 0 and self.vis_step != -1:
            ...
            if self.show:
                ...
                axs.plot(vars['r'][:,1]-vars['r_init'][:,1],'.')
                axs.plot(vars['r'][:,0]-vars['r_init'][:,0],'.')  
                axs.grid()              
                plt.show()
    ...
    def error_debug(self,vars, reused, d, dref, i):
        ...
        if i % self.err_step == 0 and self.err_step != -1:
            ...
            err += self.lam*np.linalg.norm(self.G(q))**2 
            err += np.linalg.norm(np.abs(self.D(q[np.newaxis]))-dref)**2
